...
public class MegaCannon : MonoBehaviour {
	...
	void Update () {
        ...
        Vector3 shootDirection = new Vector3(playerPosition.x - transform.position.x, playerPosition.y - transform.position.y);
        ...
        if (shootTimer >= shootFrequency)
        {
            ...
            Vector3 shootingPosition = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y + .8f, gameObject.transform.position.z);
